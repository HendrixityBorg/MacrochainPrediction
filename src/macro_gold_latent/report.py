...
def chinese_live_demo_report(payload: dict[str, Any], model_run: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "本记录演示一条固定主链从根输入、逐跳概率、链级 CI 到停止和交易动作的完整工作流。它不把通胀预期或美元作为替代路径，也不以单个尚未完成追踪的事件证明模型预测成功。", "",
        "## 1. 输入与来源", "",
        ...
        "- 字段 `root_consensus_or_nowcast` 沿用输入契约名称；本事件的数值实际是冻结开发样本的参考类中位数，不是商业市场共识。因此 `root_z` 是模型相对参考类的创新。",
        ...
        "所有系数、协方差、事件 ID 和随机种子均来自发布前绑定的 `reports/model_run.json`。`n_i` 是时间权重经 Kish 公式得到的有效证据量，不是代理数量或模拟抽样次数。", "",
    ...
    lines += [
        ...
        "内生完整传导与外部截断分别输出；现有 `q` 主要受弱正则先验约束，不应解释成已经精确识别的历史截断频率。90% CI 描述未知链概率的认识论不确定性，不是黄金收益率的价格区间。", "",
        "## 4. 每跳停止判断与认识论不确定性", "",
...
def chinese_report(
    ...
    lines += [
        ...
        "## 确认评估", "",
        f"- 同一固定拓扑的有效确认事件：{evaluation['confirmation_events']}；终点成功：{evaluation['terminal_successes']}；实际率：{evaluation['terminal_rate']:.3f}；",
    ...
    if live_payload:
        ...
        lines += [
            ...
            "这是一次可执行的弃权，不等于模型失效；后两跳仍输出但明确标为反事实。该演示证明端到端工作流与时间顺序，不以单个未决 outcome 证明预测成功。", "",
        ]
    lines += [
        "## 局限性与盲点", "",
